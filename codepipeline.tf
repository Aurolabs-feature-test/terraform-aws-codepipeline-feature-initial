...
resource "aws_codepipeline" "this" {
  ...
  stage {
    ...
    action {
      ...
      provider         = "CodeBuild"
      input_artifacts  = ["source_output"]
      output_artifacts = ["terraform_state"]
      version          = "1"
      run_order        = 1
  ...
  stage {
    ...
    action {
      ...
      owner            = "AWS"
      provider         = "CodeBuild"
      input_artifacts  = ["source_output", "terraform_state"]
      output_artifacts = ["applied_state"]
      version          = "1"
  ...
  stage {
    ...
    action {
      ...
      owner            = "AWS"
      provider         = "CodeBuild"
      input_artifacts  = ["source_output", "applied_state"]
      output_artifacts = ["destroy_plan_state"]
      version          = "1"
      run_order        = 1

      configuration = {
        ProjectName   = module.destroy_plan.codebuild_project.name
        PrimarySource = "source_output"
      }
    }
    action {
      ...
      configuration = {
        CustomData = "This action will destroy all resources in ${var.pipeline_name}. Please review the destroy plan before approving."
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      owner           = "AWS"
      provider        = "CodeBuild"
      input_artifacts = ["source_output", "destroy_plan_state"]
      version         = "1"

      configuration = {
        ProjectName   = module.destroy_apply.codebuild_project.name
        PrimarySource = "source_output"
      }
    }
  }

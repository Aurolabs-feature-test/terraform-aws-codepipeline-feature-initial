...
module "destroy_plan" {
  source                = "./modules/codebuild"
  codebuild_name        = "${var.pipeline_name}-destroy-plan"
  codebuild_role        = aws_iam_role.codebuild_execution.arn
  environment_variables = local.env_var
  build_timeout         = var.build_timeout
  build_spec            = var.build_override["destroy_plan_buildspec"] != null ? var.build_override["destroy_plan_buildspec"] : file("${path.module}/modules/codebuild/buildspecs/destroy-plan.yml")
  log_group             = aws_cloudwatch_log_group.this.name
  image                 = var.build_override["destroy_plan_image"] != null ? var.build_override["destroy_plan_image"] : "hashicorp/terraform:${var.terraform_version}"
  vpc                   = var.vpc
}

module "destroy_apply" {
  source                = "./modules/codebuild"
  codebuild_name        = "${var.pipeline_name}-destroy-apply"
  codebuild_role        = aws_iam_role.codebuild_execution.arn
  environment_variables = local.env_var
  build_timeout         = var.build_timeout
  build_spec            = var.build_override["destroy_apply_buildspec"] != null ? var.build_override["destroy_apply_buildspec"] : file("${path.module}/modules/codebuild/buildspecs/destroy-apply.yml")
  log_group             = aws_cloudwatch_log_group.this.name
  image                 = var.build_override["destroy_apply_image"] != null ? var.build_override["destroy_apply_image"] : "hashicorp/terraform:${var.terraform_version}"
  vpc                   = var.vpc
}
  ...
  statement {
    ...
    condition {
      ...
      values = [
        ...
        "arn:aws:codebuild:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:project/${var.pipeline_name}-destroy-plan",
        "arn:aws:codebuild:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:project/${var.pipeline_name}-destroy-apply"
      ]
    }
  }
}
